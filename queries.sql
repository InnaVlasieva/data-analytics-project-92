...
from employees as e
inner join sales as s on eemployees.employee_id = sales.sales_person_id
inner join products as p on sales.product_id = products.product_id
group by 
    employees.first_name || ' ' || employees.last_name,
    to_char(sales.sale_date, 'day'), extract(isodow from sales.sale_date)
...
--которую они принесли-customers_by_month
select
...
-- с покупателями первая покупка которых пришлась на время проведения
--специальных акций - special_offer
with price_null as (
    select product_id
    from products
    where price = 0
